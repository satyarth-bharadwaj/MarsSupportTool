...
test.describe("Points Component E2E Tests for input and form submission", () => {
  let browser: Browser;
  ...
  test.beforeEach(async () => {
    await page.goto("http://localhost:5173/dashboard/points"); // Adjust URL as needed
  });
...
test.describe("E2E testing by clicking the filters and check whether its getting changed", () => {
  ...
  test.beforeEach(async () => {
    await page.goto("http://localhost:5173/dashboard/points");
    const btnGroup = await page.$(".btn-group");
  ...
  test("filters are changing based on the check boxes are being clicked", async () => {
    ...
    if (selectFiltersbutton) {
      ...
      await page.waitForSelector('label:has-text("eventId")');
      const checkbox = await page.$('label:has-text("eventId")');
      ...
      if (customerInput) {
        ...
        expect(inputValue).toBe(
          "trn:tesco:uid:uuid:3e78eec4-63d3-4e1d-9502-84f92a803699"
        );
      } else {
        throw new Error("customer Input field missing");
      }
